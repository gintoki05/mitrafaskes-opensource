...
describe('SatusehatMasterWilayahAdapter', () => {
  ...
  afterEach(() => {
    if (originalBaseUrl === undefined)
      delete process.env.SATUSEHAT_MASTER_DATA_BASE_URL;
    else process.env.SATUSEHAT_MASTER_DATA_BASE_URL = originalBaseUrl;
    if (originalTimeout === undefined)
      delete process.env.SATUSEHAT_HTTP_TIMEOUT_MS;
  ...
  it('fetches the four levels through the Master Data API, not FHIR', async () => {
    ...
    const fetchMock = jest
      ...
      .mockResolvedValueOnce(
        response({
          data: [{ code: '11', parent_code: '', bps_code: '11', name: 'Aceh' }],
        }),
      )
      .mockResolvedValueOnce(
        response({
    ...
    expect(fetchMock.mock.calls[1]?.[0]).toEqual(
      new URL(
        'https://masterdata.example.test/masterdata/v1/cities?province_codes=11',
      ),
    );
    expect(fetchMock.mock.calls[2]?.[0]).toEqual(
      new URL(
        'https://masterdata.example.test/masterdata/v1/districts?city_codes=1103',
      ),
    );
    expect(fetchMock.mock.calls[3]?.[0]).toEqual(
      new URL(
        'https://masterdata.example.test/masterdata/v1/sub-districts?district_codes=110301',
      ),
    );
  });
  ...
  it('turns provider failures and invalid response into safe typed errors', async () => {
    ...
    global.fetch = jest
      .fn()
      .mockResolvedValue(response({ message: 'down' }, 503));
    await expect(adapter.fetchSnapshot()).rejects.toMatchObject<
      Partial<MasterDataProviderError>
    >({
    ...
    global.fetch = jest
      .fn()
      .mockResolvedValue(response({ data: { code: 'invalid' } }));
    await expect(adapter.fetchSnapshot()).rejects.toMatchObject({
  ...
  it('uses a gateway-safe 500-code parent batch instead of many small requests', async () => {
    ...
    expect(
      firstCityRequest.searchParams.get('province_codes')?.split(','),
    ).toHaveLength(500);
    expect(
      lastCityRequest.searchParams.get('province_codes')?.split(','),
    ).toHaveLength(1);
  });
  ...
  it('exposes a safe actionable error for provider rate limiting', async () => {
    ...
    global.fetch = jest.fn().mockResolvedValue(
      response({ message: 'too many requests' }, 429, {
        'Retry-After': '30',
      }),
    );
