...
describe('SatusehatFhirClient', () => {
  ...
  it('creates and updates Condition resources on their FHIR endpoints', async () => {
    fetchMock
      .mockResolvedValueOnce({
        ok: true,
        status: 201,
    ...
    const client = new SatusehatFhirClient(auth);
    const payload = { resourceType: 'Condition', code: { coding: [] } };

    await client.createCondition(payload);
    await client.updateCondition('condition-1', payload);
