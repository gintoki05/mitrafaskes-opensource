...
const contextRules: ProfileRule[] = [
  ...
  (record) => {
    ...
    const assignedToLocation =
      doctor.locationAssignments.length > 0
        ? doctor.locationAssignments.some(
            (assignment) =>
              assignment.locationId === record.encounter.locationId,
          )
...
const outpatientGeneralRules: ProfileRule[] = [
  ...
  requiredText(
    'anamnesis',
    'presentIllness',
    'Riwayat penyakit sekarang wajib diisi.',
  ),
  (record) =>
    record.allergyReviewStatus && record.allergyReviewStatus !== 'NOT_REVIEWED'
      ? []
      : [
          issue(
            'allergies',
  ...
  (record) =>
    record.diagnoses.filter((diagnosis) => diagnosis.isPrimary).length === 1
      ? []
      : [
          issue(
            'diagnoses',
  ...
  (record) =>
    record.diagnoses.every((diagnosis) => diagnosis.icd10Code.trim())
      ? []
      : [
          issue(
            'diagnoses',
            'diagnoses',
            'DIAGNOSIS_CODE_REQUIRED',
            'Setiap diagnosis wajib memiliki kode ICD-10.',
          ),
        ],
  (record) =>
      ...
      if (
        ...
        issues.push(
          issue(
            ...
            'DOSAGE_REQUIRED',
            'Dosis, frekuensi, dan jumlah obat harus dapat dipahami.',
          ),
        );
      }
      return issues;
  ...
  requiredText(
    'plan',
    'education',
    'Edukasi atau instruksi pasien wajib diisi.',
  ),
  requiredText(
    'plan',
    'carePlan',
    'Rencana terapi atau tindak lanjut wajib diisi.',
  ),
  (record) =>
    record.disposition
      ? []
      : [
          issue(
            ...
            'Disposisi pasien wajib dipilih.',
          ),
        ],
];
...
export function validateFinalization(
  ...
  if (!profile || String(profile.serviceProfile) !== record.serviceProfile) {
    return {
      ready: false,
