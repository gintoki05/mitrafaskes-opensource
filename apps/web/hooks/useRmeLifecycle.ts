...
export type RmeMutationState =
  "idle" | "preflighting" | "saving-draft" | "draft-saved" | "finalizing";
...
async function readResponse<T>(response: Response): Promise<T | null> {
  ...
  const body = (text ? JSON.parse(text) : null) as T | RmeApiErrorBody | null;
  if (!response.ok) {
...
function draftPayload(
  ...
  const observationDefinitions = [
    ...
    {
      field: "oxygenSaturation" as const,
      code: "2708-6",
  ...
  return {
    ...
    histories: values.histories
      .filter((history) => history.text.trim().length > 0)
      .map((history) => ({
...
export function useRmeLifecycle(encounterId: string | null) {
  ...
  const conflict =
    conflictState.encounterId === encounterId ? conflictState.conflict : null;
  ...
  const preflight = useCallback(async () => {
    ...
    try {
      ...
      if (!result)
        throw new RmeApiError("API tidak mengembalikan hasil preflight.");
  ...
  const finalize = useCallback(async () => {
    ...
    try {
      ...
      if (!finalized)
        throw new RmeApiError("API tidak mengembalikan RME final.");
      setLoadState({
        encounterId,
        record: finalized,
        error: "",
