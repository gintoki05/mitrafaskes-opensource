...
export default function ChangeRequiredPasswordScreen() {
  ...
  return <div className="mx-auto flex min-h-[calc(100vh-8rem)] w-full max-w-xl items-center justify-center px-4 py-8"><Card className="w-full"><CardHeader><CardTitle className="flex items-center gap-2 text-primary"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-primary/10"><KeyRound className="h-5 w-5" /></span>Ganti password sementara</CardTitle><p className="text-sm leading-relaxed text-muted-foreground">Untuk keamanan, password sementara harus diganti sebelum Anda dapat membuka ruang kerja klinik.</p></CardHeader><form onSubmit={submit}><CardContent className="space-y-4"><div><FieldLabel htmlFor="current-password" required>Password sementara saat ini</FieldLabel><PasswordInput id="current-password" autoComplete="current-password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} disabled={saving} /></div><div><FieldLabel htmlFor="new-password" required>Password baru</FieldLabel><PasswordInput id="new-password" autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} disabled={saving} minLength={8} /><p className="mt-1 text-xs text-muted-foreground">Minimal 8 karakter.</p></div><div><FieldLabel htmlFor="password-confirmation" required>Ulangi password baru</FieldLabel><PasswordInput id="password-confirmation" autoComplete="new-password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} disabled={saving} /></div><div className="rounded-[var(--radius-control)] border border-info/25 bg-info/10 p-3 text-xs text-info"><ShieldCheck className="mr-1 inline h-3.5 w-3.5" />Sesi lain akun ini akan dicabut setelah password berubah.</div></CardContent><CardFooter className="justify-between gap-2"><Button type="button" variant="ghost" onClick={logout} disabled={saving}><LogOut className="h-4 w-4" />Keluar</Button><Button type="submit" disabled={saving}>{saving ? 'Menyimpan…' : 'Simpan password baru'}</Button></CardFooter></form></Card></div>;
}
